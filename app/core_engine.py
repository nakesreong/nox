    ...
    def process_user_command(self, user_command_text: str, is_voice_command: bool = False) -> dict:
        ...
        if structured_nlu_result and not structured_nlu_result.get("error"):
            ...
            if intent:
                ...
                else: # Если хендлер вернул что-то странное или None (например, для "ignored" интента, если бы не было явной проверки выше)
                    if action_result_from_handler is None or (isinstance(action_result_from_handler, dict) and action_result_from_handler.get("status") == "ignored"):
                        ...
                        error_payload_for_llm = {
                            "response_type": "action_outcome", # Все равно это результат "действия" (ошибочного)
                            "success": False,
            ...
            else: # Если интент не распознан NLU (но при этом нет structured_nlu_result.get("error")) - очень редкий случай
            ...
            nlu_error_payload_for_llm = {
                "response_type": "nlu_error_report", # Сигнал для "супер-инструкции" (Сценарий 3)
                "success": False, # Явно указываем, что это неудача
                "details_or_error": f"Проблема с пониманием твоего запроса. Ошибка NLU: {structured_nlu_result.get('error')}. Детали: {structured_nlu_result.get('details', 'нет деталей')}.",
        ...
        return {
            ...
            "acknowledgement_response": acknowledgement_response, # Будет None для голосовых, если отключено
...
if __name__ == "__main__":
    print("Запуск тестового скрипта Core Engine (v_dual_response_logic_refined_ack_disabled)...") # Изменил имя для ясности
    try:
        engine = CoreEngine()
        if engine.config_data: # Убедимся, что конфиг загружен
            test_cases = [
                {"command": "включи свет", "is_voice": False, "description": "Текстовая команда - включить свет"},
                {"command": "сделай теплый свет", "is_voice": True, "description": "Голосовая команда - теплый свет (ожидаем 1 ответ, т.к. ack отключен)"}, # Изменено ожидание
                {"command": "расскажи анекдот", "is_voice": True, "description": "Голосовая команда - анекдот (ожидаем 1 ответ, т.к. ack отключен)"}, # Изменено ожидание
                {"command": "какая-то абракадабра", "is_voice": False, "description": "Текстовая команда - ошибка NLU (ожидаем 1 ответ)"}
            ]
            for case in test_cases:
                ...
                elif not result.get('acknowledgement_response'): # Если нет ни того, ни другого (для проигнорированных)
